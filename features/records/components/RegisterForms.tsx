...
import { tourOperatorNames, type Contact } from "@/features/contacts/logic";
...
function RegisterModal({ kind, locale, label, defaultBookedBy, prefill, onClose }: { kind: Kind; locale: Locale; label: string; defaultBookedBy: string; prefill?: TourPrefill; onClose: () => void }) {
  ...
  useEffect(()=>{let active=true;getContacts().then(data=>{if(active){setContacts(data);setContactsLoaded(true);}}).catch(()=>{if(active)setContactError(true);});return()=>{active=false;};},[]);
  // Operators come from the Supplier directory (Contacts, type "tour"); "other" = type a name.
  const operators = tourOperatorNames(contacts);
  const [operatorChoice, setOperator] = useState("");
  const operator = operatorChoice || operators[0] || (contactsLoaded || contactError ? "other" : "");
...
${es ? "Acción tomada" : "Action taken"}: ${formData.get("actionTaken")}`});
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="team-modal record-modal" role="dialog" aria-modal="true"><header><h2>{label}</h2><button onClick={onClose}>×</button></header><form action={submit}>{error && <p className="form-error">{error}</p>}<div className="edit-cell-form">
    ...
    {kind === "tour" && <>
      ...
      <label>{es ? "Operador" : "Operator"}<select name={operator === "other" ? undefined : "operatorName"} value={operator} onChange={e => setOperator(e.target.value)} required>{operator === "" && <option value="" disabled>{es ? "Cargando operadores…" : "Loading operators…"}</option>}{operators.map(name => <option key={name} value={name}>{name}</option>)}<option value="other">{es ? "Otros" : "Others"}</option></select>{operator === "other" && <input name="operatorName" aria-label={es ? "Nombre del operador" : "Operator name"} placeholder={es ? "Nombre del operador" : "Operator name"} maxLength={120} required />}</label><label>Tour<select name="tourName" defaultValue="Whale Watching" required>{TOUR_TYPES.map(tour => <option key={tour} value={tour}>{tour === "Other" && es ? "Otro" : tour}</option>)}</select></label>
