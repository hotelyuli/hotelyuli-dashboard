...
export function CsvImportPanel({ locale }: { locale: Locale }) {
  ...
  return <>
    <div className="csv-actions">
      <button className="primary-button" onClick={() => checkInRef.current?.click()}><Upload size={17} />{t.importCheckIns}</button>
      <button className="secondary-button" onClick={() => checkOutRef.current?.click()}><Upload size={17} />{t.importCheckOuts}</button>
