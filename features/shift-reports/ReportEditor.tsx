...
export function ReportEditor({ locale, initial, events, initialText, initialRevision, initiallyClosed, storageReady }: {
  ...
  return <div className="report-editor">
    ...
    <fieldset disabled={closed || pending}><legend>{es ? "Información de la recepción" : "Reception notes"}</legend>
      ...
      <div className="report-subhead"><h3>{es ? "Incidentes incluidos en el reporte" : "Incidents included in the report"}</h3>
      <p>{es ? "Todos los incidentes del día están incluidos; desmarque los que pertenezcan a otro turno." : "All of the day's incidents are included; untick any that belong to another shift."}</p></div>
      <div className="report-check-list">{events.map(event => <label className="report-check" key={event.id}><input type="checkbox" checked={input.eventIds.includes(event.id)} onChange={e=>update({eventIds:e.target.checked ? [...input.eventIds,event.id] : input.eventIds.filter(id=>id!==event.id)})} /><span><strong>{event.event_time.slice(0,5)} · {event.room_area ?? "—"}</strong><br />{event.description}</span></label>)}</div>
      {!events.length && <p className="report-hint">{es ? "No hay eventos registrados para esta fecha." : "No incidents recorded for this date."}</p>}
      ...
      <p className="report-hint">{es ? "Marque solo las acciones que se realizaron." : "Check only actions that were actually completed."}</p>
      <div className="report-check-list compact">{confirmations.map(([key,spanish,english])=><label className="report-check" key={key}><input type="checkbox" checked={input[key]} onChange={e=>update({[key]:e.target.checked})} />{es ? spanish : english}</label>)}</div>
    </fieldset>
    ...
    <fieldset disabled={closed || pending}><legend>{es ? "Revisión antes del cierre" : "Review before closing"}</legend><div className="report-check-list compact">{checks.map(([key,spanish,english])=><label className="report-check" key={key}><input type="checkbox" checked={input[key]} onChange={e=>update({[key]:e.target.checked})} />{es ? spanish : english}</label>)}</div></fieldset>
