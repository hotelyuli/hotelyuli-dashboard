...
export default async function TasksPage() {
  ...
  const { data: tasks } = await supabase.from("tasks").select("id,title,room_area,priority,status,assigned_to,due_at").eq("hotel_id", profile?.hotel_id ?? "").eq("operation_date", operationDate).neq("status", "completed").order("created_at", { ascending: false });
